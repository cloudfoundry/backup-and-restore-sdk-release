...
var _ = Describe("Postgres", func() {
	...
	BeforeEach(func() {
		...
		fakePgRestore17.Reset()
		fakePgRestore18.Reset()
		...
		envVars["PG_DUMP_17_PATH"] = fakePgDump17.Path
		envVars["PG_DUMP_18_PATH"] = fakePgDump18.Path
		...
		envVars["PG_RESTORE_17_PATH"] = fakePgRestore17.Path
		envVars["PG_RESTORE_18_PATH"] = fakePgRestore18.Path
	...
	Context("backup", func() {
		...
		Context("Postgres database server is version 15", func() {
			...
			Context("when pg_dump succeeds", func() {
				...
				It("takes a backup", func() {
					...
					By("not invoking the dump binary for a different version", func() {
						Expect(fakePgDump13.Invocations()).To(HaveLen(0))
					})
		...
		Context("Postgres database server is version 16", func() {
			BeforeEach(func() {
				fakePgClient.WhenCalled().WillPrintToStdOut(
			...
			Context("when pg_dump succeeds", func() {
				BeforeEach(func() {
					fakePgDump16.WhenCalled().WillExitWith(0)
				})
				...
				It("takes a backup", func() {
					...
					By("dumping the database with the correct dump binary", func() {
						...
						Expect(fakePgDump16.Invocations()).To(HaveLen(1))
						Expect(fakePgDump16.Invocations()[0].Args()).Should(ConsistOf(expectedArgs))
						Expect(fakePgDump16.Invocations()[0].Env()).Should(HaveKeyWithValue("PGPASSWORD", password))
					})
			...
			Context("when pg_dump fails", func() {
				BeforeEach(func() {
					...
					fakePgClient.WhenCalled().WillPrintToStdOut(
						" table1 \n table2 \n\n\n").
						WillExitWith(0)
					fakePgDump16.WhenCalled().WillExitWith(1)
				})
		...
		Context("Postgres database server is version 17", func() {
			BeforeEach(func() {
				fakePgClient.WhenCalled().WillPrintToStdOut(
			...
			Context("when pg_dump succeeds", func() {
				BeforeEach(func() {
					fakePgDump17.WhenCalled().WillExitWith(0)
				})
				...
				It("takes a backup", func() {
					...
					By("dumping the database with the correct dump binary", func() {
						...
						Expect(fakePgDump17.Invocations()).To(HaveLen(1))
						Expect(fakePgDump17.Invocations()[0].Args()).Should(ConsistOf(expectedArgs))
						Expect(fakePgDump17.Invocations()[0].Env()).Should(HaveKeyWithValue("PGPASSWORD", password))
					})
			...
			Context("when pg_dump fails", func() {
				BeforeEach(func() {
					...
					fakePgClient.WhenCalled().WillPrintToStdOut(
						" table1 \n table2 \n\n\n").
						WillExitWith(0)
					fakePgDump17.WhenCalled().WillExitWith(1)
				})
		...
		Context("Postgres database server is version 18", func() {
			BeforeEach(func() {
				fakePgClient.WhenCalled().WillPrintToStdOut(
					" PostgreSQL 18.6 on x86_64-pc-linux-gnu, compiled by gcc " +
						"(Ubuntu 5.4.0-6ubuntu1~16.04.12) 5.4.0 20160609, 64-bit").
					WillExitWith(0)
			})
			...
			Context("when pg_dump succeeds", func() {
				BeforeEach(func() {
					fakePgDump18.WhenCalled().WillExitWith(0)
				})
				...
				It("takes a backup", func() {
					...
					By("dumping the database with the correct dump binary", func() {
						...
						Expect(fakePgDump18.Invocations()).To(HaveLen(1))
						Expect(fakePgDump18.Invocations()[0].Args()).Should(ConsistOf(expectedArgs))
						Expect(fakePgDump18.Invocations()[0].Env()).Should(HaveKeyWithValue("PGPASSWORD", password))
					})
			...
			Context("when pg_dump fails", func() {
				BeforeEach(func() {
					...
					fakePgClient.WhenCalled().WillPrintToStdOut(
						" table1 \n table2 \n\n\n").
						WillExitWith(0)
					fakePgDump18.WhenCalled().WillExitWith(1)
				})
	...
	Context("restore", func() {
		...
		Context("Postgres database server is version 16", func() {
			...
			Context("pg_restore succeeds", func() {
				BeforeEach(func() {
					fakePgRestore16.WhenCalled().WillExitWith(0)
					fakePgRestore16.WhenCalled().WillExitWith(0)
				})

				It("calls pg_restore to get information about the restore", func() {
					Expect(fakePgRestore16.Invocations()).To(HaveLen(2))

					Expect(fakePgRestore16.Invocations()[0].Args()).To(Equal([]string{"--list", artifactFile}))
					...
					Expect(fakePgRestore16.Invocations()[1].Args()).Should(ConsistOf(expectedArgs))
					Expect(fakePgRestore16.Invocations()[1].Env()).Should(HaveKeyWithValue("PGPASSWORD", password))
				})
			...
			Context("and pg_restore fails when restoring", func() {
				BeforeEach(func() {
					fakePgRestore16.WhenCalled().WillExitWith(0)
					fakePgRestore16.WhenCalled().WillExitWith(1)
				})
		...
		Context("Postgres database server is version 17", func() {
			...
			Context("pg_restore succeeds", func() {
				BeforeEach(func() {
					fakePgRestore17.WhenCalled().WillExitWith(0)
					fakePgRestore17.WhenCalled().WillExitWith(0)
				})

				It("calls pg_restore to get information about the restore", func() {
					Expect(fakePgRestore17.Invocations()).To(HaveLen(2))

					Expect(fakePgRestore17.Invocations()[0].Args()).To(Equal([]string{"--list", artifactFile}))
					...
					Expect(fakePgRestore17.Invocations()[1].Args()).Should(ConsistOf(expectedArgs))
					Expect(fakePgRestore17.Invocations()[1].Env()).Should(HaveKeyWithValue("PGPASSWORD", password))
				})
			...
			Context("and pg_restore fails when restoring", func() {
				BeforeEach(func() {
					fakePgRestore17.WhenCalled().WillExitWith(0)
					fakePgRestore17.WhenCalled().WillExitWith(1)
				})
		...
		Context("Postgres database server is version 18", func() {
			...
			Context("pg_restore succeeds", func() {
				BeforeEach(func() {
					fakePgRestore18.WhenCalled().WillExitWith(0)
					fakePgRestore18.WhenCalled().WillExitWith(0)
				})

				It("calls pg_restore to get information about the restore", func() {
					Expect(fakePgRestore18.Invocations()).To(HaveLen(2))

					Expect(fakePgRestore18.Invocations()[0].Args()).To(Equal([]string{"--list", artifactFile}))
					...
					Expect(fakePgRestore18.Invocations()[1].Args()).Should(ConsistOf(expectedArgs))
					Expect(fakePgRestore18.Invocations()[1].Env()).Should(HaveKeyWithValue("PGPASSWORD", password))
				})
			...
			Context("and pg_restore fails when restoring", func() {
				BeforeEach(func() {
					fakePgRestore18.WhenCalled().WillExitWith(0)
					fakePgRestore18.WhenCalled().WillExitWith(1)
				})
