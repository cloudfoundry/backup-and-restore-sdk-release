...
var _ = Describe("IncrementalBucket", func() {
	...
	Describe("S3 buckets", func() {
		...
		BeforeEach(func() {
			...
			var err error
			liveBucket, err = s3bucket.NewBucket(liveBucketName, LiveRegion, S3Endpoint, creds, false, s3bucket.UsePathStyleDuringTheRefactor)
		...
		Describe("ListBlobs", func() {
			Context("without a prefix", func() {
				...
				Context("when s3 list-objects errors", func() {
					It("errors", func() {
						bucketObjectUnderTest, err := s3bucket.NewBucket("does-not-exist", LiveRegion, S3Endpoint, creds, false, s3bucket.UsePathStyleDuringTheRefactor)
						Expect(err).NotTo(HaveOccurred())
				...
				Context("when the bucket has a lot of blobs", func() {
					It("works", func() {
						bucketObjectUnderTest, err := s3bucket.NewBucket("sdk-unversioned-big-bucket-integration-test", LiveRegion, S3Endpoint, creds, false, s3bucket.UsePathStyleDuringTheRefactor)
						Expect(err).NotTo(HaveOccurred())
		...
		Describe("CopyBlobWithinBucket", func() {
			...
			Context("when the bucket was previously versioned but now is unversioned", func() {
				...
				BeforeEach(func() {
					...
					var err error
					previouslyVersionedLiveBucket, err = s3bucket.NewBucket(previouslyVersionedLiveBucketName, LiveRegion, S3Endpoint, creds, false, s3bucket.UsePathStyleDuringTheRefactor)
					Expect(err).NotTo(HaveOccurred())
				})
		...
		Describe("CopyBlobFromBucket", func() {
			...
			BeforeEach(func() {
				...
				var err error
				backupBucket, err = s3bucket.NewBucket(backupBucketName, BackupRegion, S3Endpoint, creds, false, s3bucket.UsePathStyleDuringTheRefactor)
		...
		Describe("UploadBlob", func() {
			...
			Context("when the bucket does not exist", func() {
				It("errors", func() {
					bucket, err := s3bucket.NewBucket("does-not-exist", LiveRegion, S3Endpoint, creds, false, s3bucket.UsePathStyleDuringTheRefactor)
					Expect(err).NotTo(HaveOccurred())
	...
	Describe("S3 large file test", func() {
		...
		It("works", func() {
			bucket, err := s3bucket.NewBucket(bucketName, LiveRegion, S3Endpoint, creds, false, s3bucket.UsePathStyleDuringTheRefactor)
			Expect(err).NotTo(HaveOccurred())

			srcBucket, err := s3bucket.NewBucket(PreExistingBigFileBucketName, LiveRegion, S3Endpoint, creds, false, s3bucket.UsePathStyleDuringTheRefactor)
			Expect(err).NotTo(HaveOccurred())
